...
const baseStyle =
	'fixed top-0 bottom-auto right-0 w-full transition-[transform] duration-500 ease-in-out';
...
const FullScreenNav = memo(function FullScreenNav({
	...
	return (
		<nav
			...
			<div className='relative bg-scroll bg-origin-padding'>
				<div className='w-full h-screen flex justify-end relative pointer-events-none'>
					<div
						className={`w-full bg-stone-900  z-20 md:w-[45%]  pointer-events-auto`}>
						<div
							...
							<menu className='flex flex-col w-full pointer-events-auto px-8 h-[calc(1vh*100+4rem)]'>
								{pageLinks.map((link, index) => (
									<li
										key={index}
										className={`w-full py-3 flex justify-end `}>
										<Link
											className={`link`}
