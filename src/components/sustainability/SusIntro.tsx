...
const SusIntro = ({ text, img }: IntroProps) => {
  return (
    <div className='relative before:absolute before:w-full before:h-full before:z-[1] before:bg-img-overlay overflow-hidden h-[calc((47*1vh)+2rem)]'>
      <Image src={img.url} fill alt={img.alt} className='w-full object-cover' />
      <div className='h-full flex flex-col relative justify-end px-4 sm:px-6 py-2 md:px-8 z-[2]'>
        <h1 className='my-[0.36em] text-4xl font-light md:text-5xl font-archivo tracking-wide xl:text-6xl '>
          {img.title}
        </h1>
        <div className='pb-2'>
